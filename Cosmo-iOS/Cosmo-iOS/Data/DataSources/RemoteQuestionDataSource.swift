...
class RemoteQuestionDataSourceImpl: RemoteQuestionDataSource {
    private let maxRetryCount = 3
        ...
        private func createMessages(for subject: String) -> [[String: Any]] {
            ...
            Each question must have exactly 4 choices, and the answer must be an integer between 1 and 4 (corresponding to the choice index, 1-based). All questions and choices must be written in Korean (Hangul). Return only the JSON array, do not include any additional text, explanations, or formatting outside the JSON array (e.g., no ```json markers). Strictly adhere to the specified format. The difficulty level should be intermediate or higher. Ensure the response is a valid JSON array.
            """
    ...
    private func parseQuestions(from text: String, subject: String) -> [Question] {
        ...
        do {
            // 임시 구조체 대신 Question의 속성에 맞춘 타입으로 디코딩
